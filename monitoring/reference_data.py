"""
Phase 4, Step 1: Build the "reference" dataset for drift detection AND
retraining.
...
This same file also serves as retraining/retrain.py's "original knowledge"
sample (combined with newly streamed data to retrain). Keeping ONE small
reference file (~20K rows, a few MB) instead of committing the full 60MB
features_sample.parquet to git keeps the repo lean enough for GitHub
Actions to check out quickly on every scheduled run.
...
# + units_sold, which retrain.py needs as the training target
FEATURE_COLS = [
...
COLS_TO_SAVE = FEATURE_COLS + ["units_sold"]
...
def main():
    ...
    df = pd.read_parquet(FEATURES_PATH, columns=COLS_TO_SAVE + ["is_simulated"])
